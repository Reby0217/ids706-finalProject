...
def generate_forms_file():
    with open(
        "api_generator/forms/forms_structure", "r"
    ) as forms_structure_file:
        forms_structure = forms_structure_file.read()

    with open(
        "api_generator/forms/library_imports", "r"
    ) as library_imports_file:
    ...
    forms = "\n\n".join(
        base_form.format(model_name=model_name)
...
def generate_routes_file():
    with open(
        "api_generator/routes/routes_structure", "r"
    ) as routes_structure_file:
        routes_structure = routes_structure_file.read()

    with open(
        "api_generator/routes/library_imports", "r"
    ) as library_imports_file:
        library_imports = library_imports_file.read()
    ...
    project_imports = base_imports.format(
        ...
        forms_name=", ".join(
            list(
                map(
                    lambda model_name: f"{model_name}Form",
                    API_GENERATOR.values(),
                )
            )
        ),
    )
